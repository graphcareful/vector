    ...
    FS::File: Unpin,
{
    ...
    pub(crate) async fn flush_pending_finalizers(&mut self) -> io::Result<()> {
        ...
        // Count the buffered bytes before `flush_inner` publishes them to the reader, for the same
        // reason as `flush`: otherwise the reader can consume and acknowledge the record (decrementing
        // `total_buffer_size`) before the increment lands, underflowing the counter.
        self.flush_write_state();
        self.flush_inner(true).await?;
        Ok(())
    }
