    ...
    FS::File: Unpin,
{
    ...
    pub async fn next(&mut self) -> Result<Option<T>, ReaderError<T>> {
        ...
        // (e.g. the sink task is torn down with the request in flight during a graceful shutdown)
        // would otherwise finalize as `Dropped`, which the batch treats as a no-op and leaves at
        // the optimistic `Delivered` default -- silently promoting the read-head for a record that
        // was never delivered. Use a pessimistic notifier so such a drop reports `Errored` instead,
        // mirroring the write path's `Errored`-on-drop guard. Only for real deliveries: during the
        // recovery-seek replay the records are already-acknowledged re-reads that are intentionally
        // discarded, so they keep the optimistic notifier and must not be reported as failures.
