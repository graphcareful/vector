...
///   source → disk buffer → S3 batch → drain on reload → new buffer → S3 delivery resumes
///
/// This proves that changing a disk buffer's `max_size` and triggering a config
/// reload (the same code path the `-w` file-watcher uses) does NOT stall, and
/// that events sent both before and after the reload are delivered to S3.
...
async fn s3_disk_buffer_reload_delivers_all_events() {
    ...
    // Build topology: basic_source → disk buffer → S3 sink
    ...
    // Only the disk buffer max_size changes; the source serialises identically
    // so it will be reused (not rebuilt) during the reload.
    ...
    loop {
        ...
        let count: usize = {
            let mut n = 0;
            for key in keys {
                n += get_lines(get_object(&bucket, key).await).await.len();
            }
            n
        };
