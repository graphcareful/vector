...
//! faults beforehand. Fault injection stops instantly, but in-flight requests and
//! retries may still be settling, so the command waits before judging. An oversized
//! record is expected to be dropped, but it must not crash Vector or permanently
//! block its disk buffer.
