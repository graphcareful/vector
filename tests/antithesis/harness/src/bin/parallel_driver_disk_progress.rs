...
//! flowing through the disk buffer while network and clock faults are active, so
//! the buffer's fill/retry paths are exercised before the terminal check judges
