...
#   SCENARIO_FAULTS_ENABLED whether to enable this repository's custom node/CPU/clock faults; defaults to 1
...
FAULTS_ENABLED="${FAULTS_ENABLED:-$SCENARIO_FAULTS_ENABLED}"
...
# timers.
FAULTS=()
if [[ "$FAULTS_ENABLED" == "1" ]]; then
  # Network faults (default-on for every non-excluded container) and clock jitter
  # stress timers and the buffer's fill/retry/recover path without crashing a
  # process, so they never tear a disk-buffer data file.
...
cmd=(snouty launch
  ...
  --source "$SOURCE"
  --duration "$DURATION"
  # ${arr[@]+…} guards the expansion: FAULTS is empty when faults are disabled,
  # and a bare "${FAULTS[@]}" under `set -u` aborts on bash < 4.4 (e.g. macOS 3.2).
  ${FAULTS[@]+"${FAULTS[@]}"}
  "$@")
